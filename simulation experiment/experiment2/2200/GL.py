# -*- coding: utf-8 -*-
"""
Created on Mon Oct 16 22:17:47 2023
...
y =  X[:,:110].dot(w1) + X[:,110:220].dot(w1) + X[:,220:330].dot(w1) + 0.1*siga
w = np.hstack((np.hstack((w1,w1,w1)),np.zeros(1870)))
groups = np.arange(X.shape[1]) // 110
p = Calc_prob(y)
clssify = pred(p)
...
shuffle(X1[:141])
shuffle(X1[141:])
Y = np.hstack((np.ones(141),np.zeros(159))) #generate labels of original dataset 
...
w21 = np.hstack((np.hstack((w2,w2,w2)),np.ones(1870)))
W_i = np.diag(w21)
...
auc = []
for i in range(100):
    gl = GroupLasso(groups=groups,group_reg=0.04893900918477494,l1_reg=0,frobenius_lipschitz=True,scale_reg="none",
                subsampling_scheme=None,supress_warning=True,n_iter=1000,tol=1e-3,)
    gl.fit(X_train, Y_train)
    ab = gl.coef_
